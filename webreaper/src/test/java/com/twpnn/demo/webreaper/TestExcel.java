...
public class TestExcel {
	...
	public static void main(String[] args) {
		...
		TestExcel myReport = new TestExcel();
	...
	public void createExcel() {
		...
		try {
			...
			fileOut = new FileOutputStream("myReport.xlsx");
			wb.write(fileOut);
			fileOut.close();
	...
	private int insertHeaderInfo(Sheet sheet, int index) {
		...
		c = row.createCell(0);
		c.setCellValue("Main url:");
		c.setCellStyle(csTopLeft);
		c = row.createCell(1);
		c.setCellStyle(csTop);
		c = row.createCell(2);
		c.setCellValue("www.ba.no");
		c.setCellStyle(csTopRight);

		rowIndex++;
		row = sheet.createRow(rowIndex);
		c = row.createCell(0);
		c.setCellValue("Download time:");
		...
		c.setCellValue("HTTP response code:");
		c.setCellStyle(csLeft);
		c = row.createCell(1);
